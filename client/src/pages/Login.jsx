...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = error.response?.data?.message || 'Failed to log in. Please check your credentials.';
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex justify-center items-center bg-[#0b0c10] py-12 px-4 min-h-[calc(100vh-140px)]">
      <div className="card w-full max-w-4xl p-0 shadow-2xl flex flex-col md:flex-row overflow-hidden border border-white/5 bg-zinc-900/80 backdrop-blur-xl">
        ...
        {/* Image Section */}
        <div className="md:w-1/2 bg-emerald-900/20 relative hidden md:block border-r border-white/5">
          <img 
        ...
        <div className="md:w-1/2 p-8 md:p-12">
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-xl bg-emerald-600/10 border border-emerald-500/20 mb-4 shadow-inner">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex justify-between">
              ...
              <Link to="/forgot-password" className="text-sm text-emerald-500 hover:text-emerald-400 transition-colors">Forgot password?</Link>
            </div>
            <div className="relative">
        ...
        <div className="mt-8 text-center text-zinc-500">
          <p>Don't have an account? <Link to="/register" className="text-emerald-500 font-bold hover:text-emerald-400 transition-colors">Sign up for free</Link></p>
        </div>
        </div>
      </div>
    </div>
