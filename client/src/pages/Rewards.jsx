...
export default function Rewards() {
  const [balance, setBalance] = useState(1200); // Mock User Balance
  ...
  const rewards = [
    ...
    { id: 2, title: '20% Off Next Cab', desc: 'Get a heavy discount on your next Electric Cab ride.', cost: 300, icon: <FaTicketAlt className="text-amber-500"/>, type: 'Discount', img: 'https://images.unsplash.com/photo-1620050854499-4d6cb6007ec1?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80' },
  ...
  const handleRedeem = (reward) => {
    if (balance >= reward.cost) {
      ...
      toast.error('Insufficient EcoCoins for this reward.');
    }
  };
  ...
  return (
    <div className="bg-[#0b0c10] min-h-screen py-12">
      <div className="container mx-auto px-4 max-w-5xl">
        ...
        {/* Wallet Header */}
        <div className="bg-zinc-900 rounded-3xl p-8 md:p-12 text-white shadow-2xl shadow-blue-900/10 mb-12 relative overflow-hidden flex flex-col md:flex-row items-center justify-between border border-white/5">
           ...
           <div className="absolute -left-10 -top-10 text-blue-500/10 text-[200px]"><FaCoins /></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {rewards.map((reward, index) => (
            <motion.div 
              ...
              key={reward.id} 
              className="bg-zinc-900/50 backdrop-blur-md p-0 rounded-3xl shadow-lg border border-zinc-800 flex flex-col hover:border-zinc-700 hover:shadow-2xl transition-all overflow-hidden group"
            >
              {/* Image Banner */}
              ...
              <div className="p-6 flex-grow flex flex-col">
                <span className="text-xs font-bold uppercase tracking-widest text-blue-500 mb-2 block">{reward.type}</span>
        ...
        {/* Referral Banner */}
        <div className="mt-16 bg-blue-600/10 border border-blue-500/20 rounded-3xl p-8 flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden backdrop-blur-xl">
          <FaGift className="absolute -right-8 -bottom-8 text-[150px] text-blue-500/10" />
          <div className="relative z-10 w-full md:w-2/3">
            <h3 className="text-2xl font-bold text-white mb-2">Invite Friends, Earn Faster</h3>
            <p className="text-blue-200 mb-6 text-lg">Get 200 EcoCoins for every friend who signs up and completes their first green ride.</p>
            <div className="flex shadow-lg rounded-xl overflow-hidden max-w-md">
              <input type="text" readOnly value="https://greencommute.app/ref/johndoe" className="bg-zinc-900/80 border-y border-l border-zinc-700 px-4 py-3 w-full text-sm text-zinc-400 focus:outline-none font-medium tracking-wide" />
              <button onClick={() => toast.success('Link copied to clipboard!')} className="bg-blue-600 text-white px-6 py-3 font-bold text-sm hover:bg-blue-500 transition-colors">Copy</button>
            </div>
          </div>
        </div>
